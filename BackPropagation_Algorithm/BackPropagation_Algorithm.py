...
class Player:
    ...
    def think(self):
        self.decision = self.brain.forward(torch.tensor(self.vision))
...
class Population(Player):
    ...
    def Backpropagation(self):
        ...
        self.generation += 1
        top_candidates = Old_Players[:5]
        NextBatch_Players = []

        for player in Old_Players:
            player.optimizer.zero_grad()
            vision_tensor = torch.tensor(player.vision, dtype=torch.float32, requires_grad=True)
            decision_tensor = player.brain(vision_tensor.unsqueeze(0)) 
            target_tensor = torch.tensor([[-player.fitness]], dtype=torch.float32, requires_grad=False)
            loss = Function.mse_loss(decision_tensor, target_tensor)  # Compute Mean Squared Error loss
            loss.backward()  # Compute gradients
            player.optimizer.step()
            new_player = Player()
            new_player.brain = player.brain
            NextBatch_Players.append(new_player)

        self.players = NextBatch_Players
        for player in self.players:
            player.fitness = 0
    ...
    def saveTopBirds(self, top_candidates):
        save_folder = 'BackPropagation_Algorithm\Best_Models'
    ...
    def plot_statistics(self):
        ...
        plt.legend()
        plt.savefig('BackPropagation_Algorithm/top_scores.png')  # Save the plot as an image
        plt.close()
        ...
        plt.legend()
        plt.savefig('BackPropagation_Algorithm/avg_lifespans.png')  # Save the plot as an image
        plt.close()
