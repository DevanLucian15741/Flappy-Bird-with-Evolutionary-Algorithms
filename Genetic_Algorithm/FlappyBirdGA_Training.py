...
class Population(Player):
    ...
    def CrossoverFunction(self, Paren1_NN, Parent2_NN):
        Child_NN = NeuralNetwork(3)
        for child_param, parent1_param, parent2_param in zip(Child_NN.parameters(),Paren1_NN.parameters(), Parent2_NN.parameters()):
            child_param.data.copy_((parent1_param+parent2_param)/2)
        return Child_NN
    
    def MutationFunction(self, brain):
        MutationProbability = 0.05
        MutationScale = 0.5
        for param in brain.parameters():
            if random.random() < MutationProbability:
                param.data += torch.randn_like(param.data) * MutationScale

    def GeneticNaturalSelection(self):
        solution = self.players
        self.Selection_Sort_Players(solution)
        self.generation += 1
        top_candidates = solution[:5]
        new_generation = []

        for _ in range(self.size):
            parent1, parent2 = random.sample(top_candidates, 2)# Apply roullte selection here
            child_brain = self.CrossoverFunction(parent1.brain, parent2.brain)
            self.MutationFunction(child_brain)
            new_player = Player()
            new_player.brain = child_brain
    ...
    def saveTopBirds(self, top_candidates):
        save_folder = 'Genetic_Algorithm\Best_Models'
        for i, player in enumerate(top_candidates, start=1):
            filename = os.path.join(save_folder, f"top_model_{i}_generation.pt")
    ...
    def plot_statistics(self):
        ...
        plt.legend()
        plt.savefig('top_scores.png')  # Save the plot as an image
        plt.close()
        ...
        plt.legend()
        plt.savefig('avg_lifespans.png')  # Save the plot as an image
        plt.close()
...
def main():
    ...
    while True:
        ...
        if not population.extinct():
            ...
            population.GeneticNaturalSelection()
